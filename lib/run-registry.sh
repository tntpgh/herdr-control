...
# Same ALTER-guarded shape as v3/v4: a brand-new database gets both columns
# from CREATE TABLE IF NOT EXISTS; an existing v4/v5 db may need either added.
_migrate_schema_v5() {
  local has_project has_manifest
  has_project=$(_sql "SELECT 1 FROM pragma_table_info('tasks') WHERE name='project';" 2>/dev/null)
  if [ -z "$has_project" ]; then
    _sql "ALTER TABLE tasks ADD COLUMN project TEXT NOT NULL DEFAULT '';" >/dev/null 2>&1
  fi
  has_manifest=$(_sql "SELECT 1 FROM pragma_table_info('tasks') WHERE name='manifest';" 2>/dev/null)
...
# manifest (15th, OPTIONAL) is the canonical capability-manifest JSON from
